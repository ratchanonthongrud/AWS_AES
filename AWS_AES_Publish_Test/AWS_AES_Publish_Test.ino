...
void encrypt(char * plainText, char * key, unsigned char * outputBuffer){
  ...
  mbedtls_aes_setkey_enc( &aes, (const unsigned char*) key, strlen(key) * 8 );//Sets the encryption key and returns 0 upon success
...
void loop() {
  ...
  if (now - lastMsg > 2000) {
    ...
    snprintf (msg, BUFFER_LEN, "%s", Test.c_str());
    ...
    Serial.println(strlen(payload2));
    
    if(client.publish("Test_AES128", payload2) == true){// Publishes payload and returns true upon success
      Serial.println("\nPublish Success!");
    }
    else{
      Serial.println("\nPublish Failed!");
