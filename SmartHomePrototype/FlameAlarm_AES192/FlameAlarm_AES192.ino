...
void encrypt(char * plainText, char * key, unsigned char * outputBuffer){
  Serial.println("Strat Encrypt:");
  mbedtls_aes_context aes;
  mbedtls_aes_init( &aes );
  
  if( mbedtls_aes_setkey_enc( &aes, (const unsigned char*) key, strlen(key) * 8 ) == 0){//Sets the encryption key and returns 0 upon success
      Serial.println("Sets the encryption key Success!");
    }
  else{ 
      Serial.println("Sets the encryption key Failed!"); 
    }
   
  if( mbedtls_aes_crypt_ecb( &aes, MBEDTLS_AES_ENCRYPT, (const unsigned char*)plainText, outputBuffer) == 0){//Encrypt and returns 0 upon success
...
void loop() {
  char * key = "abcdefghijklmnopqrstuvwx"; //กำหนดขนาด key
  unsigned char cipherTextOutput[32];
  String Ciphertext; 
  String Ciphertext2; 
  String Zero = "0";
  
  readFlame = digitalRead(FlamePIN); // Detect Flame form DHT sensor
  ...
  if (now - lastMsg > 2000) {
    lastMsg = now;
    //=============================================================================================
    String AlarmFLAME = "Detect FLAME!";
    String NoFLAME = "No FLAME";
    if (readFlame == 0)
  {
    Serial.println(AlarmFLAME);
    snprintf (msg, BUFFER_LEN, "%s", AlarmFLAME.c_str());
    ...
    if(client.publish("FlameAlarm_AES192", payload2) == true){// Publishes payload and returns true upon success
      Serial.println("\nPublish Success!");
    }
    else{
      Serial.println("\nPublish Failed!");
