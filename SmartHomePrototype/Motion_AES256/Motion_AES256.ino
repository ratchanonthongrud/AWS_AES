...
void loop() {
  char * key = "abcdefghijklmnopqrstuvwxyz123456"; //กำหนดขนาด key
  unsigned char cipherTextOutput[32];
  String Ciphertext; 
  String Ciphertext2; 
  String Zero = "0";
    
  readMotion = digitalRead(MotionPin);
  ...
  if (now - lastMsg > 2000) {
    lastMsg = now;
    //=============================================================================================
    String AlarmMotion = "Detect Motion!";
    String NoMotion = "No Motion";
     if (readMotion == 1){
      Serial.println(AlarmMotion);
      snprintf (msg, BUFFER_LEN, "%s", AlarmMotion.c_str());
    ...
    if(client.publish("Motion_AES256", payload2) == true){// Publishes payload and returns true upon success
      Serial.println("\nPublish Success!");
    }
    else{
      Serial.println("\nPublish Failed!");
